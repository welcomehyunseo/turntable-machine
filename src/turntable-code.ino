...
void turnMotor(int motor) 
{
  ...
  if
     (motor == MOTOR3_DISK) 
    {
...

      {
        ...
        if
           ((digitalRead(SENSOR3_1) ) || (digitalRead(SENSOR3_2))) 
          {
  ...
  if
     (motor == MOTOR4_CUT) 
    {
      ...
      while
         (digitalRead(SENSOR4) == LOW) 
        {

          Serial.println("motor4 cut is running");

          if
             (digitalRead(SENSOR4) == HIGH) 
            {
